...
term:
    ...
    | LPAREN LPAREN UNDERSCORE EXTRACT finish = INT start = INT RPAREN t = term RPAREN  { A.ExtractOperation (mk_pos $startpos, start, finish, t) }
...
sort: 
    | id = ID                                                                           { A.AmbiguousType (mk_pos $startpos, id) }
    | INTT                                                                              { A.IntType (mk_pos $startpos) }
    | BOOLT                                                                             { A.BoolType (mk_pos $startpos) }
    | REALT                                                                             { A.RealType (mk_pos $startpos) }
    | LPAREN UNDERSCORE BITVECT i = INT RPAREN                                          { A.BitVecType (mk_pos $startpos, i) }                                         
    | LPAREN s = sort sort_list = nonempty_list(sort) RPAREN                            { A.MultiSort (mk_pos $startpos, s, sort_list) }
...
constant: 
    ...
    | LPAREN UNDERSCORE v = BVCONST size = INT RPAREN                                   { A.BitVecConst (mk_pos $startpos, v, size) }